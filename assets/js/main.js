...
const classCircle = document.getElementsByClassName('circle');
if (app_features.length > 0) {
    Array.from(app_features).forEach((element) => {
        element.addEventListener('mouseover', (e) => {
            if (e.target.tagName === 'A' || e.target.parentElement.tagName === 'A' && classCircle.length > 0) {
...
const menu_child = document.querySelectorAll('.js-anchor-link');

if (typeof(menu_child) != 'undefined' && menu_child != null) {
    menu_child.forEach(el =>
